# SiteModels.ps1 - Site entry and data store classes
# This file must be loaded AFTER DeviceModels.ps1, SiteUtilities.ps1

# Extended validation utility with device validation
class ExtendedValidationUtility : ValidationUtility {    
...
class SiteDataStore {
    ...
    if (Test-Path $this.DataFile) {
            try {
                ...
                foreach ($item in $jsonData) {
                    ...
                    $site.SiteSubnet = $item.SiteSubnet
                    ...
                    # Device counts
                    $site.SwitchCount = if ($item.SwitchCount) { $item.SwitchCount } else { 1 }
                    $site.APCount = if ($item.APCount) { $item.APCount } else { 1 }
                    $site.UPSCount = if ($item.UPSCount) { $item.UPSCount } else { 1 }
                    $site.CCTVCount = if ($item.CCTVCount) { $item.CCTVCount } else { 1 }
                    $site.PrinterCount = if ($item.PrinterCount) { $item.PrinterCount } else { 1 }
                    
                    # Load device arrays
                    if ($item.Switches) {
                        foreach ($switchData in $item.Switches) {
                            $switch = [SwitchInfo]::new()
                            $switch.ManagementIP = Get-SafeValue $switchData.ManagementIP
                            $switch.Name = Get-SafeValue $switchData.Name
                            $switch.AssetTag = Get-SafeValue $switchData.AssetTag
                            $switch.Version = Get-SafeValue $switchData.Version
                            $switch.SerialNumber = Get-SafeValue $switchData.SerialNumber
                            $site.Switches.Add($switch)
                        }
                    }
                    
                    if ($item.AccessPoints) {
                        foreach ($apData in $item.AccessPoints) {
                            $ap = [AccessPointInfo]::new()
                            $ap.ManagementIP = Get-SafeValue $apData.ManagementIP
                            $ap.Name = Get-SafeValue $apData.Name
                            $ap.AssetTag = Get-SafeValue $apData.AssetTag
                            $ap.Version = Get-SafeValue $apData.Version
                            $ap.SerialNumber = Get-SafeValue $apData.SerialNumber
                            $site.AccessPoints.Add($ap)
                        }
                    }
                    
                    if ($item.UPSDevices) {
                        foreach ($upsData in $item.UPSDevices) {
                            $ups = [UPSInfo]::new()
                            $ups.ManagementIP = Get-SafeValue $upsData.ManagementIP
                            $ups.Name = Get-SafeValue $upsData.Name
                            $ups.AssetTag = Get-SafeValue $upsData.AssetTag
                            $ups.Version = Get-SafeValue $upsData.Version
                            $ups.SerialNumber = Get-SafeValue $upsData.SerialNumber
                            $site.UPSDevices.Add($ups)
                        }
                    }
                    
                    if ($item.CCTVDevices) {
                        foreach ($cctvData in $item.CCTVDevices) {
                            $cctv = [CCTVInfo]::new()
                            $cctv.ManagementIP = Get-SafeValue $cctvData.ManagementIP
                            $cctv.Name = Get-SafeValue $cctvData.Name
                            $cctv.SerialNumber = Get-SafeValue $cctvData.SerialNumber
                            $site.CCTVDevices.Add($cctv)
                        }
                    }
                    
                    if ($item.PrinterDevices) {
                        foreach ($printerData in $item.PrinterDevices) {
                            $printer = [PrinterInfo]::new()
                            $printer.ManagementIP = Get-SafeValue $printerData.ManagementIP
                            $printer.Name = Get-SafeValue $printerData.Name
                            $printer.Model = Get-SafeValue $printerData.Model
                            $printer.SerialNumber = Get-SafeValue $printerData.SerialNumber
                ...
                Show-MessageBox "Error loading site data: $_" "Data Load Error" "OK" "Warning"
                $this.Entries = [System.Collections.Generic.List[SiteEntry]]::new()
                $this.SaveData()
            }
        } else {
    ...
    try {        
        ...
        Show-MessageBox "Error saving site data: $_" "Data Save Error" "OK" "Error"
    }
}
}