...
$xamlFile = Join-Path (Split-Path -Parent $MyInvocation.MyCommand.Path) "NetworkManagement.xaml"

# Import the Import/Export module (includes class definitions)
try {
    ...
    if (Test-Path $importExportPath) {
        ...
        $altPath = Join-Path (Join-Path $scriptPath "Site Network Identifier") "SiteImportExport.ps1"
        if (Test-Path $altPath) {
            . $altPath
        }
        else {
            $errorMsg = "SiteImportExport.ps1 not found at:`n$importExportPath`nor at:`n$altPath"
...
function Show-EditSiteWindow {
    ...
    try {
        ...
        $editXamlFile = Join-Path $scriptPath "EditSiteWindow.xaml"
